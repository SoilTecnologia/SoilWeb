...
class UpdateSchedulingAngleUseCase{
    constructor(
        ...
        async execute (schedulingangle: SchedulingAngleModel){
            const getSchedulingAngle = await this.schedulingAngleRepository.findById(schedulingangle.schedulingangle_id);
            

            if(getSchedulingAngle){
                const newSchedulingAngle = await this.schedulingAngleRepository.update(schedulingangle);
