...
class GetPivotSchedulingHistoryController {
  async handle(req: Request, res: Response, next: NextFunction) {
    const { id } = req.params;
    const getPivotSchedulingHistoryUseCase = container.resolve(
      GetPivotSchedulingHistoryUseCase
    );

    try {
      const allSchedulingHistoryfromPivot =
        await getPivotSchedulingHistoryUseCase.execute(id);
