...
class DeleteSchedulingHistoryController {
  async handle(req: Request, res: Response, next: NextFunction) {
    const { id } = req.params;
    const deleteSchedulingHistoryUseCase = container.resolve(
      DeleteSchedulingHistoryUseCase
    );

    try {
      const allSchedulingsHistoryFromPivots =
        await deleteSchedulingHistoryUseCase.execute(id);
