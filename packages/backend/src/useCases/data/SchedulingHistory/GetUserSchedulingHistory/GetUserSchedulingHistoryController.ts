...
class GetUserSchedulingHistoryController {
  async handle(req: Request, res: Response, next: NextFunction) {
    const { id } = req.params;
    const getUserSchedulingHistoryUseCase = container.resolve(
      GetUserSchedulingHistoryUseCase
    );

    try {
      const allSchedulingHistoryfromUser =
        await getUserSchedulingHistoryUseCase.execute(id);
