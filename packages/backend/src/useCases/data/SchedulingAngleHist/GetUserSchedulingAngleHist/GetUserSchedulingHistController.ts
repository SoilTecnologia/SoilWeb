...
class GetUserSchedulingAngleHistController {
  async handle(req: Request, res: Response, next: NextFunction) {
    const { id } = req.params;
    const getUserSchedulingAngleHistUseCase = container.resolve(
      GetUserSchedulingAngleHistUseCase
    );

    try {
      const allSchedulingAngleHistfromUser =
        await getUserSchedulingAngleHistUseCase.execute(id);
