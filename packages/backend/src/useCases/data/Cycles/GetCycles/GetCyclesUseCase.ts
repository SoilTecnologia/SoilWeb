...
class GetCyclesUseCase {
  ...
  async execute(pivot_id: StateModel['pivot_id'], start: string, end: string) {
    ...
    const states = await this.applyQueryGetStoryCycle(
      pivot_id,
      startDate,
      endDate
    );
  ...
  */
    if (!states || states.length <= 0) {
      ...
      for (let state of states!!) {
        ...
        const variables = await this.applyQueryGetVariableGroupBt(
          state.state_id
        );
        ...
        for (let variable of variables) {
          if (variable.percentimeter !== null) {
            if (
              oldPercent === null ||
              !isPercentDiferent(oldPercent, variable.percentimeter!)
            ) {
              console.log(
                `Percent maior antigo antigo: ${oldPercent} Novo: ${variable.percentimeter!}`
              );
              ...
              if (percentimeters && percentimeters.length > 0) {
                ...
                const percentEquals = isPercentDiferent(
                  lastItem.value,
                  variable.percentimeter
                );
                !percentEquals &&
                  percentimeters.push({
              ...
              console.log(
                `Intervalo de  percent curto ${oldPercent} Novo: ${variable.percentimeter!}`
              );
            }
