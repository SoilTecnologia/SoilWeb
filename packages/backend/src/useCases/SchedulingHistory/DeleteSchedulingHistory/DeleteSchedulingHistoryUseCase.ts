...
class DeleteSchedulingHistoryUseCase {
  ...
  private async applyQueryDelete(scheduling_history_id: string) {
    try {
      return await this.schedulingHistoryRepository.delete(scheduling_history_id);
    } catch (err) {
      messageErrorTryAction(
        err,
        true,
  ...
  async execute(scheduling_history_id: SchedulingHistoryModel['scheduling_history_id']) {
    const schedulinghistory = await this.applyQueryFindById(scheduling_history_id);

    if (!schedulinghistory) throw new Error('Scheduling History does not exist');
    const deletedScheduleHistory = await this.applyQueryDelete(scheduling_history_id);
    console.log(`Historico de agendamento excluído com sucesso do banco de dados....`);
    console.log('....');
