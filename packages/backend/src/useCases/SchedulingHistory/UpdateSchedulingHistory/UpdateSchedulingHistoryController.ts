...
class UpdateSchedulingHistoryController {
  async handle(req: Request, res: Response, next: NextFunction) {
    ...
    const updateSchedulingHistoryUseCase = container.resolve(UpdateSchedulingHistoryUseCase);
    try {
      const putSchedulingHistory = await updateSchedulingHistoryUseCase.execute(schedulingHistory);
