...
router.post('/create', async (req, res, next) => {
  const { farm_id, pivot_name, lng, lat, start_angle, end_angle, radius } =
    req.body;
...
router.put('/update', async (req, res, next) => {
  ...
  try {
    const updatedPivot = await updatePivotController(pivot_id, farm_id, pivot_name, lng, lat, start_angle, end_angle, radius);
