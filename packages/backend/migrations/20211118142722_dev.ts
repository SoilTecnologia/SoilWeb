...
export async function up(knex: Knex): Promise<void> {
  return knex.schema
    ...
    .createTable('farms', (table) => {
      table.uuid('farm_id').primary().defaultTo(knex.raw('(uuid_generate_v4())'));
    ...
    .createTable('nodes', (table) => {
      table.uuid('node_id').primary().defaultTo(knex.raw('(uuid_generate_v4())'));
    ...
    .createTable('pivots', (table) => {
      table.uuid('pivot_id').primary().defaultTo(knex.raw('(uuid_generate_v4())'));
      ...
      table.uuid('node_id').references('node_id').inTable('nodes').index().notNullable();
    })
    .createTable('states', (table) => {
      table.uuid('state_id').primary().defaultTo(knex.raw('(uuid_generate_v4())'));
      table.boolean('power');
      table.boolean('water');
    ...
    .createTable('state_variables', (table) => {
      table.uuid('state_variable_id').primary().defaultTo(knex.raw('(uuid_generate_v4())'));
      table.float('angle');
