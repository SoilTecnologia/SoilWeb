...
const MapComponent = () => {
  const { farm, setPivot, socketPayload, setSocketPayload } = useContextUserData();
  ...
  useEffect(() => {
    ...
    const update = async () => {
      ...
      socketPayload.map(async (payload) => {
        if (payload) {
          const findUpdatedPivot = pivotMapList.pivots.find((pivot) => pivot.pivot_id == payload.pivot_id ? pivot : null)
          console.log(payload.type, socketPayload)
          if (payload.type == 'status') {
            await map.setPaintProperty(`${payload.pivot_id}`, "fill-color", stateSelector(payload))
            await map.setLayoutProperty(`${payload.pivot_id}+image`, "icon-image", directionIndicator(payload));
          } else if (payload.type == 'variable') {
            await map.setLayoutProperty(`${payload.pivot_id}+image`, "icon-rotate", payload.angle);
          }
        }
